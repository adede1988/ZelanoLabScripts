function out = ica_flag_spikes(data, spikeDat,spikeProm, varargin)
...
p.addParameter('Highpass', 30, @(x) isempty(x) || (isscalar(x) && x>0));
...
% -------------------- Spike candidates from original data --------------------
%% ADDED: run your detector and collapse across channels to a sample-wise mask
% If your detect_spikes signature is (data, thr, win, markWhole, requireFull):
% spikeFlags = detect_spikes(data, S.spikeThresh, S.spikeWin, false);  % C x T x N (tune params)
