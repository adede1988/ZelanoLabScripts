...
<root>/<id>/preProc/<id>_<task>PreProc.mat        ("intermediate")
   ...
   │     • assemble_outDat_all(S,P)    (load intermediate/raw -> outDat)
...
| Kind | Files |
|---|---|
| **Config / loaders (shared)** | `labPaths.m`, `applyParams.m`, `writeParams.m`, `writePreProcX.m`, `assemble_outDat_all.m` |
| **Shared pipeline (do not edit per task)** | `downsample_data.m`, `preprocess_eeg.m`, `preprocess_macros.m`, `preprocess_respiration_wholetrace.m`, `detect_sniffs_from_TTLs.m`, `refine_onsets_with_phase.m`, `plot_sniff_epochs.m`, `paramCheck.m`, `behDatFromSniffs.m`, `parSave.m` |
| **Task-specific (rewrite for a new task)** | `<task>_makeOutDat.m`, `build_behavior_table_<task>.m`; O15: `detect_ttls_O15.m`; breathing: `process_respiration_breathing.m`, `alignTargetBreathingTraceSimplify.m`, `processECG.m`/`buildECGz.m`/`paramCheckECG.m`, `flagBadBreaths.m`, `plotBreathLengths.m` |
...
changes. Adding a whole new task means a new `<task>_makeOutDat.m`, a
`build_behavior_table_<task>.m`, an `assemble_outDat_all` case, and a `_main.m`
modeled on an existing one — see the tutorial.
