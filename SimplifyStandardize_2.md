...
- [x] verify that all functions are either completely task specific or completely shared.
  — Every assemble function is now wholly task-specific (`assembleRaw_*`, `assembleOutDat_O15extras`, `detect_ttls_O15`) or wholly shared (`assembleOutDat`, `loadIntermediateRaw`, `resolveFigDir`). **Each main now calls its own loader directly** (`raw = assembleRaw_<task>(S); outDat = assembleOutDat(raw,S,P);`, O15 also `assembleRaw_O15`+`assembleOutDat_O15extras`) with inline `<-- TASK-SPECIFIC` markers, so a naive reader sees exactly what to edit without opening other files. No function in the pipeline flow branches on task. `assemble_outDat_all` is kept only as a backward-compat one-call wrapper for the `_dev/run_*` harnesses.
