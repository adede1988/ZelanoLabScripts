function run_thresh_tasks123(sessFilter, doSaveBack, csvPath)
% RUN_THRESH_TASKS123  FOOOF / bestMac stage (analysis "Task 1") for all sessions.
%   Per thresh session: FOOOF the macBP channels, pick the bestMac channel, write
%   bestMac into the .mat, save the per-subject FOOOF periodic-spectrum figure,
%   and (over)write the per-channel rows of threshTask_fooof_summary.csv.
%
%   run_thresh_tasks123(sessFilter, doSaveBack, csvPath)
%     sessFilter : cellstr/string of sessIDs ([] = all on disk)
%     doSaveBack : true -> write outDat.bestMac into the .mat (safe temp+rename)
%     csvPath    : output CSV ([] = groupStatFigs\threshTask_fooof_summary.csv)
%
...
%   run_thresh_fooof_one (which appends/replaces just that session's rows).
    ...
    for i = 1:height(T)
        ...
        try
            ...
            figDir = fullfile(L.figPath, id, 'threshTask');
