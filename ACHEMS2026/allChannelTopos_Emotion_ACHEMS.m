...
test = cellfun(@(x) length(x)>0, strfind({chanFiles.name}, 'emotional'));
chanFiles = chanFiles(test); 
...
% --- left axis: voltage ---
yyaxis left
plot(x(x>6 & x<9.5), smoothdata(yRaw(x>6 & x<9.5), 'gaussian', 5), '-', 'Color', rawCol, 'LineWidth', 2);
% plot(x, yHP,  '-', 'Color', hpCol,  'LineWidth', 1);
ylim([-75 75])
ylabel('voltage (\muV)', ...
    'FontSize', 20, ...
...
% --- right axis: respiration ---
yyaxis right
plot(x(x>6 & x<9.5), yResp(x>6 & x<9.5), '-', 'Color', rspCol, 'LineWidth', 6);
ax.YColor = labCol;
yyaxis left
ax.YColor = labCol;
...
outFile = fullfile(figSaveDir, 'controlPassiveChirp.jpg');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);

[hFig, ~] = plotPeakFreq_bySubjectBlocks(conIDXall, allSubIDs, allFlatSpec, ...
    bgCol, labCol, conColors)

outFile = fullfile(figSaveDir, 'controlPassiveChirpSingle.jpg');
...
outFile = fullfile(figSaveDir, 'conChirp_downChirp.png');
...
outFile = fullfile(figSaveDir, 'conChirp_NotAmpDriven.png');
...
outFile = fullfile(figSaveDir, 'controlStackedPhase_template.png');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);
[hFig, subTab, countMat, binCtrDeg] = plot_gamRspPhase_circStackedHist( ...
    conIDXall, allSubIDs, allBehDat, conColors, labCol, bgCol, 'gamRspPACphase', 15);
outFile = fullfile(figSaveDir, 'controlStackedPhase_hilbert.png');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);

[hFig, subTab, countMat, binCtrDeg] = plot_gamRspPhase_circStackedHist( ...
    dup1IDX, allSubIDs, allBehDat, cols, labCol, bgCol, 'gamPeakidx50', 15);
outFile = fullfile(figSaveDir, 'dup1StackedPhase_template.png');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);
[hFig, subTab, countMat, binCtrDeg] = plot_gamRspPhase_circStackedHist( ...
    dup1IDX, allSubIDs, allBehDat, cols, labCol, bgCol, 'gamRspPACphase', 15);
outFile = fullfile(figSaveDir, 'dup1StackedPhase_hilbert.png');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);

[hFig, subTab, countMat, binCtrDeg] = plot_gamRspPhase_circStackedHist( ...
    dup2IDX, allSubIDs, allBehDat, cols, labCol, bgCol, 'gamPeakidx50', 15);
outFile = fullfile(figSaveDir, 'dup2StackedPhase_template.png');

exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);
[hFig, subTab, countMat, binCtrDeg] = plot_gamRspPhase_circStackedHist( ...
    dup2IDX, allSubIDs, allBehDat, cols, labCol, bgCol, 'gamRspPACphase', 15);
outFile = fullfile(figSaveDir, 'dup2StackedPhase_hilbert.png');
...
outFile = fullfile(figSaveDir, 'dup1Chirp_downChirp.png');
...
outFile = fullfile(figSaveDir, 'dup2Chirp_downChirp.png');
exportgraphics(hFig, outFile,'BackgroundColor', bgCol, 'Resolution', 300);
