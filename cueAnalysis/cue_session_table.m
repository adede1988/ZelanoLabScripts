function T = cue_session_table(verbose)
% CUE_SESSION_TABLE  Cheap manifest of cue-task sessions (no multi-GB loads).
%   T = cue_session_table()  returns a table with one row per cue session that
%   has a final preproc .mat on disk, with identity/group info and cheap file
%   metadata (channel count, sample count, origFS) read via h5info.
%
...
%   the 4-group means).
    ...
    for i = 1:n
        id = ids{i};
        sessID(i) = string(id);
        f = fullfile(roots{i}, id, 'preProc', [id '_cueTaskPreproc.mat']);
        if ~isfile(f)
            % case-insensitive / fuzzy fallback
            d = dir(fullfile(roots{i}, id, 'preProc', [id '*cueTask*.mat']));
    ...
    if verbose
        Td = T(T.onDisk,:);
        fprintf('\nCue sessions listed: %d | on disk: %d\n', height(T), height(Td));
