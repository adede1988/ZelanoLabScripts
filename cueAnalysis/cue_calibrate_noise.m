function cue_calibrate_noise()
% CUE_CALIBRATE_NOISE  Calibrate the RELATIVE sharp-deflection threshold.
%   For each session's bestMac: d = 10ms max-min; robust z zd=(d-median)/(1.4826*MAD)
%   over the whole recording; per trial (epoch [-1.75,5.75]s) take max(zd). Sweep K
%   and report the dataset-wide fraction of trials that would be rejected (max zd>K),
%   plus the worst single-session fraction, so we can pick K for >0%, <=20% loss.
    ...
    fprintf('\n  K   overall%%rej   worstSess%%   #sess@100%%   HRM%%\n');
    hrm = sess=="240923_OBE_NMH_HRM";
    for K = [4 5 6 7 8 9 10 12 15 20]
        rej = z>K;
        perSess = arrayfun(@(s) mean(rej(sess==s))*100, usess);
        hrmr = NaN; if any(hrm), hrmr = mean(rej(hrm))*100; end
        fprintf('  %-4g  %8.1f     %8.1f    %6d       %.0f\n', K, mean(rej)*100, max(perSess), sum(perSess>=100), hrmr);
    end
end
