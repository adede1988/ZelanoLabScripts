function plot_session_singletrials(sessID, outSub, epWin)
% PLOT_SESSION_SINGLETRIALS  One figure PER TRIAL of a session's bipolar (macBP)
%   channels, trialStart-locked, stacked. bestMac is drawn slightly bolder and
%   the RELATIVE sharp-deflection windows (zd>K, the same rule cue_noise_trials
%   uses for rejection) are overlaid in red on bestMac. The title says whether
%   the whole trial was rejected.
%
%   plot_session_singletrials(sessID [, outSub, epWin])
%   Saves <figs>\<id>\cueTask\<outSub>\trial_NN.png (one per trial).
    ...
    T = cue_session_table(false); T = T(strcmp(T.sessID, sessID), :);
    if isempty(T), error('plot_session_singletrials:noSess', 'session not on disk: %s', sessID); end
    ...
    macLabs = labs(macIdx);
    if isempty(macIdx), error('plot_session_singletrials:noMacBP', '%s has no macBP channels', sessID); end
    ...
    fs = od.fs; ts = round(od.TTL.trialStart); nTr = numel(ts);
    ...
    NT = cue_noise_trials(bestSig, fs, ts, epWin);     % uses calibrated K default
    times = NT.times;

    figDir = fullfile(L.figPath, sessID, 'cueTask', outSub);
    ...
    for t = 1:nTr
        ...
        xlabel('Time from trialStart (ms)'); ylabel('bipolar channels (raw uV, stacked)');
