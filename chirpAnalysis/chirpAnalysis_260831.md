...
- Goal is to analyze across all four available tasks: `cueTask`, `threshTask`, `O15`, and `breathingTask`
...
- if `bestMac` is not included, then assess which bipolar referenced macro channel has the highest power in the 25-58 Hz range after flattening the power spectrum through aperiodic removal and select that channel. Add the `bestMac` field to the preproc file and resave it. 
...
- **cueTask** Split analysis by trial type `behDat.type`. Specifically, analyze 'hit', 'cr', and 'noCue' trials separately.
