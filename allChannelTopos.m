...
fig = plotFlatSpec_bySubjectBlocks(conIDXall, allSubIDs, allFlatSpec); 
title('controls')
...
figure; 
tim = .002:.002:12; 
plot(tim(130:end), conRawDat(133,130:end), 'color', 'k')
hold on 
plot(tim(130:end), highpass(conRawDat(133,130:end), 20, 500), 'color', 'blue', 'linewidth', 1)
yyaxis right
plot(tim(130:end), smoothdata(conRawRsp(133,130:end), 'gaussian', 30),...
    'linewidth', 3)
...
% figure; 
% tim = .002:.002:12; 
% plot(tim, conRawDat(139,:))
% yyaxis right
% plot(tim, conRawRsp(139,:))
% 
% 
% for ii = 1:575
%     if max(conRawDat(ii,:))<100 && min(conRawDat(ii,:))>-100
%         figure; 
...
hFig  = plot_phaseDev110_vs_HRV(conIDXall, allSubIDs, allBehDat)
hFig = plot_phaseDev110_vs_HRV(dup1IDX, allSubIDs, allBehDat)
hFig = plot_phaseDev110_vs_HRV(dup2IDX, allSubIDs, allBehDat)
...
hFig = topo_fromBehVarStem(allSubIDs, allBehDat, conIDX, 'theta_theta_phase')
